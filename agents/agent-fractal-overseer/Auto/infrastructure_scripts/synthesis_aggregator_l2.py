...
def correlate_insights(aggregated_data):
    ...
    """
    bio_nodes = [d for d in aggregated_data if d["niche"] == "bio-wealth"]
    fin_nodes = [d for d in aggregated_data if d["niche"] == "financas"]
...
def run_synthesis_aggregation():
    ...
    print(f"Aggregating data from {len(active_nodes)} nodes using ThreadPoolExecutor (max_workers=50)...")
    
    aggregated_data = []
    
    # Concurrency to meet the <10s SLO even with 500+ nodes
    ...
    report = {
        "summary": {
            ...
            "slo_status": "COMPLIANT" if synthesis_latency < 10 else "BREACHED",
    ...
    with open(FINAL_REPORT_MD, "w") as f:
        ...
        f.write(f"- **Synthesis Latency:** **{report['summary']['synthesis_latency_sec']:.2f}s** (Target: <10s)\n")

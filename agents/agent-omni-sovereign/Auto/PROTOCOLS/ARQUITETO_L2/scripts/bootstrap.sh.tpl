...
if [ "$NICHE_NAME" == "financas" ] || [ "$NICHE_NAME" == "finanças" ]; then
    ASSET_NAME="Bot de Arbitragem de Liquidez em DEX"
    ASSET_TAG="FIN-DEX-01"
elif [ "$NICHE_NAME" == "bio-wealth" ]; then
    ASSET_NAME="Aggregator de Bio-Data"
    ASSET_TAG="BIO-AGGR-01"
else
    ASSET_NAME="Asset SaaS de Alta Margem ($NICHE_NAME)"
    ASSET_TAG="SAAS-GEN-01"
fi
...
global:
  scrape_interval: 15s
scrape_configs:
  - job_name: 'active-asset'
    static_configs:
      - targets: ['active-asset:3000']
EOF

# Install Arbitrage Engine requirements
pip3 install requests

# Setup Arbitrage Engine (Governing the new Active Asset - V2.1 Hardened)
...
# Updated for 99.99% SLO per Protocol ENTIDADE_12
...
def run_cycle(prometheus_url, app_dir, service_name):
    # Using 1m windows for sub-60s response/failover
    availability = query_prometheus(prometheus_url, "sum(rate(http_requests_total{status!~'5..'}[1m])) / sum(rate(http_requests_total[1m]))")
    latency = query_prometheus(prometheus_url, "rate(http_request_duration_seconds_sum[1m]) / rate(http_request_duration_seconds_count[1m])")
    req_rate = query_prometheus(prometheus_url, 'sum(rate(http_requests_total[1m]))')
    ...
    if (latency is not None and latency > LATENCY_UPPER_THRESHOLD) or (availability is not None and availability < AVAILABILITY_THRESHOLD):
        print(f"Soberania Alert: Scaling up {service_name} due to performance.")
        ...
        print(f"Soberania Alert: Scaling down {service_name} for efficiency.")
        set_scale(app_dir, service_name, current_scale - 1)

if __name__ == "__main__":
    while True:
        run_cycle("http://localhost:9090", "/opt/devops-squad", "active-asset")
        time.sleep(15) # 15s interval for rapid failover
PYEOF

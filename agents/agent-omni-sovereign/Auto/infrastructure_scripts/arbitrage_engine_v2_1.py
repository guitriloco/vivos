...
def run_arbitrage_cycle(prometheus_url, app_dir, service_name):
    log_msg = f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] Starting Arbitrage Engine V2.1 Cycle for {service_name}..."
    print(log_msg)
    
    # 1. Gather Metrics (Fast 1m ratios for 99.99% SLO)
    availability = query_prometheus(prometheus_url, "empire_asset:availability:ratio_1m")
    latency = query_prometheus(prometheus_url, "empire_asset:latency_p95:seconds_1m")
    req_rate = query_prometheus(prometheus_url, f'sum(rate(http_requests_total{{job="{service_name}"}}[1m]))')
    ...
    # Log
    with open(f"/home/team/shared/scripts/arbitrage_v2_1_{service_name}.log", "a") as f:
        f.write(f"{log_msg}\n{metrics_log}\nAction: {action_taken}\n")

def main():
    parser = argparse.ArgumentParser(description="Empire Arbitrage Engine V2.1")

...
# Import ProfitEngine para recuperação inteligente
try:
...
class AutomationCore:
    ...
    async def wf04_cart_abandoned(self, email: str, name: str, cart_value: float, behavior: Dict = None) -> WorkflowResult:
        """
        WF-04: Carrinho Abandonado Inteligente
        1. Adicionar tag carrinho_abandonado
        2. Gera email personalizado com Gemini
        3. Agenda recuperação
        """
        self.log("WF-04", f"Carrinho abandonado inteligente: {email} | Valor: {cart_value}")
        ...
        try:
            ...
            # 2. IA Recovery (Gemini)
            niche_val = self.niche.value if hasattr(self.niche, 'value') else str(self.niche)
            ai_content = await self.profit_engine.generate_recovery_email(name, niche_val, behavior or {})
            
            # 3. Email 1h (Com conteúdo da IA)
            self._schedule_email(
                email, name,
                template="ai_recovery",
                subject=ai_content.get("subject", "Esqueceu algo? 🔥"),
                body=ai_content.get("body", ""),
                send_at=datetime.now() + timedelta(hours=1)
            )
            result.actions_triggered.append("ai_recovery_email_scheduled")

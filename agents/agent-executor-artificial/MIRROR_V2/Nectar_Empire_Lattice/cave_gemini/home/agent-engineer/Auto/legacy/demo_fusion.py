...
"""
Script de Demonstração da Fusão Claude-Gemini
Local: FUSAO_EXTREMA/demo_fusion.py
"""
...
# Adiciona o diretório FUSAO_EXTREMA ao path para importação
sys.path.append(str(Path(__file__).parent))

# Corrige os imports relativos
...
def demo_fusion_capabilities():
    """
    Demonstra as capacidades do sistema de fusão
    """
    print("🚀 INICIANDO DEMONSTRAÇÃO DA FUSÃO CLAUDE-GEMINI 🚀\n")
    ...
    # Lista de prompts de demonstração para diferentes tipos de tarefas
    demo_prompts = [
        {
            "prompt": "Como otimizar um algoritmo de ordenação em Python?",
            "expected_type": "technical"
        },
        {
            "prompt": "Crie uma história inovadora sobre IA e humanos colaborando",
            "expected_type": "creative"
        },
        {
            "prompt": "Analise os prós e contras de diferentes arquiteturas de microserviços",
            "expected_type": "analytical"
        },
        {
            "prompt": "Como resolver o problema de vazamento de memória em aplicações Node.js?",
    ...
    for i, item in enumerate(demo_prompts, 1):
        print(f"--- Demonstração {i} ---")
        print(f"Prompt: {item['prompt']}")
        print(f"Tipo esperado: {item['expected_type']}")

        # Executa a fusão
        result = orchestrator.execute_fusion_process(item['prompt'])

        print(f"Tipo detectado: {result['task_type']}")
        print(f"Score de sinergia: {result['fusion_result']['synergy_score']:.2f}")
        print(f"Partes da resposta fundida: {len(result['fusion_result']['fused_response'].split('[')) - 1}")
        print()

    # Demonstração de conselhos
    print("💡 CONSELHOS PARA MELHORES RESULTADOS:")
    print(orchestrator.get_advice_for_best_results())

def run_simple_test():
    """
    Executa um teste simples para verificar funcionamento básico
    """
    print("🔧 TESTE BÁSICO DE FUNCIONAMENTO\n")

    try:
        orchestrator = FusionOrchestrator()
        result = orchestrator.execute_fusion_process("Qual é a capital do Brasil?")

        print("✅ Teste realizado com sucesso!")
        print(f"   Tipo de tarefa: {result['task_type']}")
        print(f"   Score de sinergia: {result['fusion_result']['synergy_score']:.2f}")
        print()
...
if __name__ == "__main__":
    print("🧪 Demonstração do Sistema de Fusão Claude-Gemini")
    print("="*60)

    # Executa teste básico primeiro
    run_simple_test()

    # Executa demonstração completa
    demo_fusion_capabilities()

    print("="*60)
    print("✨ Demonstração concluída com sucesso!")
    print("Agora você está pronto para usar o sistema de fusão Claude-Gemini!")